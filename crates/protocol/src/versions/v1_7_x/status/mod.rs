    ...
    impl PacketId for ServerboundStatusRequest {
        fn packet_id(ver: u32) -> u8 {
            debug_assert_eq!(
                ...
                "ServerboundStatusRequest is only defined for 1.7.10 (protocol {})",
                super::PROTOCOL_VERSION
            );
            0x00
        }
    ...
    impl PacketId for ServerboundPingRequest {
        fn packet_id(ver: u32) -> u8 {
            debug_assert_eq!(
                ver,
                super::PROTOCOL_VERSION,
                "ServerboundPingRequest is only defined for 1.7.10 (protocol {})",
                super::PROTOCOL_VERSION
            );
            0x01
        }
    ...
    impl PacketId for ClientboundStatusResponse {
        fn packet_id(ver: u32) -> u8 {
            debug_assert_eq!(
                ver,
                super::PROTOCOL_VERSION,
                "ClientboundStatusResponse is only defined for 1.7.10 (protocol {})",
                super::PROTOCOL_VERSION
            );
            0x00
        }
    ...
    impl PacketId for ClientboundPongResponse {
        fn packet_id(ver: u32) -> u8 {
            debug_assert_eq!(
                ver,
                super::PROTOCOL_VERSION,
                "ClientboundPongResponse is only defined for 1.7.10 (protocol {})",
                super::PROTOCOL_VERSION
            );
            0x01
        }
...
mod tests {
    ...
    fn status_response_roundtrip() {
        let p = ClientboundStatusResponse {
            json_response: r#"{"version":{"name":"1.7.10","protocol":5},"players":{"max":20,"online":0},"description":{"text":"A Minecraft 1.7.10 server"}}"#.to_string(),
        };
        let mut buf = BytesMut::new();
    ...
    #[test]
    fn packet_ids_are_correct_for_1_7_10() {
