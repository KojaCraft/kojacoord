    ...
    fn decode(src: &mut Bytes) -> Result<Self, ProtocolError> {
        ...
        if src.remaining() < addr_len {
            return Err(ProtocolError::Io(std::io::Error::new(
                ...
                "Missing bytes while reading ServerboundHandshake address",
            )));
        }
        ...
        let server_address = String::from_utf8(addr_bytes).map_err(|_| {
            ProtocolError::Io(std::io::Error::new(
                ...
                "Invalid UTF-8 string in ServerboundHandshake address",
            ))
        })?;
        ...
        if src.remaining() < 2 {
            return Err(ProtocolError::Io(std::io::Error::new(
                std::io::ErrorKind::UnexpectedEof,
                "Missing bytes for ServerboundHandshake port",
            )));
        }
